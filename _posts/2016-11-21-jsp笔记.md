...
2,在web.xml配置<pre><code><session-timeout> 10 </session-timeout></code></pre>单位是分钟<br>
<hr>
**application对象**<br>
...
<pre><jsp:useBean><jsp:setProperty><jsp:getProperty>
...
<code><jsp:useBeans></code><br>
作用：在jsp页面中实例化或者在指定范围内使用javabean：<br>
<code><jsp:useBean id="标示符" class="java类名（包含包名）" scope="作用范围"/></code><br>
<br>
<code><jsp:setProperty></code><br>
作用：给应实例化的javabean对象的属性赋值，一共四中形式。<br>
<pre><jsp:setProperty name = "javaBean实例名" property="*"/>(跟表单关联)
...
<jsp:setProperty name = "javaBean实例名" property="propertyName" param="request对象中的参数名"/>(跟request参数关联)</pre><br>
...
<code><jsp:getProperty name="javaBean实例名" property="属性名"/></code><br>
<hr>
**javabean的四个作用域范围**<br>
